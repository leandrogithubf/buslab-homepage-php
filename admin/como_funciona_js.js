...
function preTableComo_funciona(){
		$("#limit").change(function(){
	         $("#pagina").val(1);
	         dataTableComo_funciona();
	    });
       ...
       $('.table').on("click",".ordemUp",function(e){
            var params = {
                idcomo_funciona: $(this).attr("codigo") 
            }

            $.post(
                'como_funciona_script.php?opx=alteraOrdemCima',
                params,
                function(data){
        ...
        $('.table').on("click", ".ordemDown", function(e){
                ...
                $.post(
                    'como_funciona_script.php?opx=alteraOrdemBaixo',
                    params,
                    function(data){
                        var resultado = new String (data.status);
	...
	var myColumnDefs = [
		{key:"idcomo_funciona", sortable:true, label:"ID", print:true, data:true},
...
function dataTableComo_funciona(){
	    limit = $("#limit").val();
        pagina = $("#pagina").val();
               ...
               success:function(data){
                      ...
                      if(data.totalRecords > 0){
                           $.each(data.records, function(index, value){
                                ...
                                tr += '<a href="index.php?mod=como_funciona&acao=formComo_funciona&met=editComo_funciona&idu='+value.idcomo_funciona+'"><img src="images/ico_edit.png" height="16" width="16" alt="ico" /><div class="tt"><span class="one">Editar</span><span class="two"></span></div></a>';
                                tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.nome+' ?\',\'php\', \'como_funciona_script.php?opx=deletaComo_funciona&idu='+value.idcomo_funciona+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
                                tr += '</div></td>';
                           });
...
$(document).ready(function(){
	$(".img").change(function(){
        ...
        formData.append('tipo_como_funciona', $(this).attr("tipo"));             
        formData.append('idcomo_funciona', $("#idcomo_funciona").val());
        if(tipo == "logo"){ 
