...
?>
<link rel="stylesheet" type="text/css" href="vantagens_css.css" />
<script type="text/javascript" src="vantagens_js.js"></script>
...
<?php if($_REQUEST['acao'] == "formVantagens"){
	if($_REQUEST['met'] == "cadastroVantagens"){
		...
		$action = "vantagens_script.php?opx=cadastroVantagens";
		$metodo_titulo = "Cadastro de Vantagens";
		$idVantagens = 0 ;
	...
	if($_REQUEST['met'] == "editVantagens"){
		
		if(!verificaPermissaoAcesso($MODULOACESSO['modulo'].'_editar', $MODULOACESSO['usuario'])) {
			header('Location:index.php?mod=home&mensagemalerta='.urlencode('Voce nao tem privilegios para acessar este modulo!'));
			exit;
		}
		$action = "vantagens_script.php?opx=editVantagens";
		$metodo_titulo = "Editar Vantagens";
		$idVantagens = (int) $_GET['idu'];
		$vantagens = buscaVantagens(array('idvantagens'=>$idVantagens));

		if (count($vantagens) != 1) exit;
		$vantagens = $vantagens[0];

        $StringIcone = strlen($vantagens['icone']);
        if ($StringIcone > 3) {
            $FontAwesome = false;
        } else {
            $FontAwesome = true;
            $icones_Edit = buscaFW3(array('idfw' => $vantagens['icone']));
...
?>
	...
	<div id="titulo">
		...
		<ul class="other_abs">
			<li class="other_abs_li"><a href="index.php?mod=vantagens&acao=listarVantagens">Listagem</a></li>
			<li class="others_abs_br"></li>
			<li class="other_abs_li"><a href="index.php?mod=vantagens&acao=formVantagens&met=cadastroVantagens">Cadastro</a></li>
		</ul>
	</div>
  
	<div id="principal">
		<form class="form" name="formVantagens" id="formVantagens" method="post" action="<?php echo $action; ?>" enctype="multipart/form-data">

			<div id="informacaoVantagens" class="content">
				<div class="content_tit">Dados Vantagens:</div>
                    <!-- ========== Upload Icone ========== -->
                    <div class="box_ip" style="width:100%">
                        ...
                        <div id="tab-1" class="tab-content <?= $FontAwesome ? 'current' : ''; ?>">
                            ...
                            <?php if ($_GET['met'] == 'editVantagens') : ?>
                                <div id="mostrar_icone" style="margin: 15px">
                                    <i class='fa fa-<?= $icones_Edit['nome']; ?> fa-2x '></i>
                                    <input type="hidden" name="icone" value="<?= $FontAwesome ? $icones_Edit['idfw'] : $vantagens['icone']; ?>" id="imagem_icone">
                                </div>
                            <?php else : ?>
                                <div id="mostrar_icone" style="margin: 15px">
                        ...
                        <div id="tab-2" class="tab-content <?= !$FontAwesome ? 'current' : ''; ?>">
                            ...
                            <img class="pump" src="<?= $caminho . $vantagens['icone']; ?>" height='87' id="icone" style="margin-top: 10px;display: <?= $_GET['met'] == 'editVantagens' ? (!empty($vantagens['icone'] && !$FontAwesome) ? 'initial' : 'none') : 'none'; ?>"><br>
					...
					<div class="box_ip" style='width:50%'>
						...
						<input type="text" name="titulo" class="required" id="nome" size="30" maxlength="255" value="<?php echo $vantagens['titulo']; ?>"/>
					</div>
               <div class="box_ip" style='width:50%;'>
                        ...
                        <div class="box_sel_d" style='width:99%;'>
                            <select name="ididiomas" id="ididiomas" class='required'>
                                <?php foreach($idiomas as $key => $idioma):?>
                                    <option value="<?=$idioma['ididiomas']?>" <?php print ($vantagens['ididiomas'] == $idioma['ididiomas'] ? ' selected="selected" ' : ''); ?> > <?=$idioma['idioma']?> </option>
                                <?php endforeach;?>
                            </select>
                        </div>
                   </div>
					...
					<div class="box_ip box_txt">
						...
						<textarea name="descricao" class="required" id="descricao" cols="34" rows="4" ><?php echo $vantagens['descricao']; ?></textarea>
					</div>
			</div> 

			<input type="hidden" name="idvantagens" id="idvantagens" value="<?= $idVantagens; ?>" />
...
<?php if($_REQUEST['acao'] == "listarVantagens"){ ?><?php
...
?>
	...
	<div id="titulo">
		<i class="fa fa-asterisk" aria-hidden="true"></i>
		<span>Listagem de Vantagens</span>
		<ul class="other_abs">
			<li class="other_abs_li"><a href="index.php?mod=vantagens&acao=listarVantagens">Listagem</a></li>
			<li class="others_abs_br"></li>
			<li class="other_abs_li"><a href="index.php?mod=vantagens&acao=formVantagens&met=cadastroVantagens">Cadastro</a></li>
	...
	<div id="principal" >
		<div id="abas"> 
			<ul class="abas_list">
				<li class="abas_list_li action"><a href="javascript:void(0)">Vantagens</a></li>
			</ul>
			<ul class="abas_bts">
				<li class="abas_bts_li"><a href="index.php?mod=vantagens&acao=formVantagens&met=cadastroVantagens"><img src="images/novo.png" alt="Cadastro de Vantagens" title="Cadastrar Vantagens" /></a></li>
				<li class="abas_bts_li"><a href="javascript:void(0)" onclick="popUp('relatorio_class.php?modulo=vantagens&output=print&'+queryDataTable);"><img src="images/imprimir.png" alt="Imprimir listagem" title="Imprimir listagem"></a></li>
				<li class="abas_bts_li"><a href="javascript:void(0)" onclick="popUp('relatorio_class.php?modulo=vantagens&output=xls&'+queryDataTable);"><img src="images/excel.png" alt="Exportar para o Excel" title="Exportar para o Excel"  ></a></li>
			</ul>
		</div>
		...
		<script type="text/javascript">
			...
			requestInicio = "tipoMod=vantagens&p="+preventCache+"&";
			ordem = "ordem";
			dir = "asc";
			$(document).ready(function(){
				preTableVantagens();
			});
			dataTableVantagens('<?php print $buscar; ?>');
			columnVantagens();
		</script> 
