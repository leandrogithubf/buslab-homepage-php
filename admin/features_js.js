          ...
          success:function(data){
                ...
                if(data.totalRecords > 0){
                    ...
                    $.each(myColumnDefs, function(col, ColumnDefs){
                      ...
                      tr += '<a href="#" onclick="wConfirm(\'Excluir Registro\',\'Tem certeza que deseja excluir o registro '+value.nome+' ?\',\'php\', \'features_script.php?opx=deletaFeatures&idu='+value.idfeatures+'\');"><img src="images/ico_del.png" height="17" width="17" alt="ico" /><div class="tt"><span class="one">Excluir</span><span class="two"></span></div></a>';
                      tr += '</div></td>';
                    }); 
...
$(document).ready(function(){
   ...
   $("#ididiomas").change(function(){
      var options = "";
      $.ajax({
         url: 'atuacao_script.php?opx=buscaAtuacao',
         type: 'post',
         dataType: 'json',
         ...
         success: function (e) {
            ...
            $(e.dados).each(function(index,value){
               options += "<option value="+value.idatuacao+">"+value.nome+"</option>";
            });
            $("#idatuacao").html(options);
         }
      })
   });
   ...
   $("#tipo").change(function(){
      if($(this).val() == 2){
         ...
         $("#numero").hide();
         $("#numero2").attr('name','numero');
         $("#numero2").addClass('required');
         $("#numero2").show();
      }
      else if($(this).val() == 1){
         $("#numero").val($("#numero2").val());
         $("#numero2").attr('name','');
         $("#numero2").removeClass('required');
         $("#numero2").hide();
         $("#numero").attr('name','numero');
  	...
  	$(".bt_save").click(function(event){
        ...
        if($("#imagem_icone").val() == '' && $("#imagem_value").val() == ''){
          ...
          $("#inputArquivoBotao").css("border", "solid 0px red");
    		...
    		$('#formFeatures').find('.required').each(function(){
...
$(".bt_cancel").click(function(e){
  ...
  if($("#mod").val() == 'cadastro' && $('#idfeatures').val() != 0){
      $.ajax({
          url:'features_script.php',
...
//////////////////////////////////////////
    ...
    $("#content-image").on("click",".cropImagem", function(e){
        ...
        pasta = "features"; 
        idrelacao = $("#idfeatures").val();
        width = 175;
        height = 130;
        nome_imagem = $(this).closest("li").find("input[name='imagem_features[]']").val();
        $.ajax({
  ...
  $(".btExcluirImagem").click(function(e){
        ...
        idfeatures = $("#formFeatures").find("#idfeatures").val();
        idfeatures_imagem = $("#"+idPosicao).find("input[name='idfeatures_imagem[]']").val();
        imagem = $("#"+idPosicao).find("input[name='imagem_features[]']").val();
        ...
        $.ajax({
            ...
            data:
            {
              ...
              imagem:imagem,
              idfeatures_imagem:idfeatures_imagem,
              descricao: post2
            },
            beforeSend:function(){
              $.fancybox.showLoading();
...
function enviaImagensAjax(input, posicao, limite){ 
  ...
  formData.append('idfeatures', $("#idfeatures").val());
  ...
  $.ajax({
   ...
   //SE DER TUDO CERTO NO AJAX TEMOS QUE MUDAR ALGUMAS COISAS NOS "appends" ANTERIORES
    ...
    success:function(data){ 
        if(data.status == true){ 
            $li = '<li class="ui-state-default'+posicao+' move" id="'+posicao+'" idimagem="'+data.idfeatures_imagem+'">';
            $li += '<img id="img'+posicao+'" class="imagem-gallery" style="opacity:1;" src="'+data.caminho+'">';
            $li += '<a class="editImagemDescricao" idimagem="'+data.idfeatures_imagem+'" href="#"><button class="edit"></button></a>';
            $li += '<a class="excluirImagem" idimagemdelete="'+data.idfeatures_imagem+'" href="#"><button class="delete"></button></a>';
            //$li += '<a class="postImagem" idimagempost="'+data.idfeatures_imagem+'" href="#"><button class="post_imagem"></button></a>'; 
            //$li += '<a class="cropImagem" idImagemCrop="'+data.idfeatures_imagem+'"><button class="crop_imagem"></button></a>'; 
            $li += '<input type="hidden" name="idfeatures_imagem[]" value="'+data.idfeatures_imagem+'">'; 
            $li += '<input type="hidden" name="descricao_imagem[]" value="">'; 
            $li += '<input type="hidden" name="imagem_features[]" value="'+data.nome_arquivo+'">';
            $li += '<input type="hidden" name="posicao_imagem[]" value="'+posicao+'">';
            $li += '</li>'; 
            $("#sortable").append($li); 
            $("#idfeatures").val(data.idfeatures); 
            if(numImagem > 1){
